...
import { guestCancelBooking } from '@/lib/api';
import type { BookingDetail } from '@/types/api';

export function GuestCancellationAction({ booking, onChange }: {
  ...
  async function cancelBooking() {
    ...
    setError('');
    try {
      onChange(await guestCancelBooking(booking.id, cancellation.policy_version, reason));
      setOpen(false);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Цуцлах хүсэлтийг баталгаажуулж чадсангүй. Дахин оролдоно уу.');
    } finally {
  ...
  return (
    <>
      <Pressable accessibilityRole="button" onPress={() => { setAccepted(false); setError(''); setOpen(true); }} style={styles.openButton}>
          ...
          <KeyboardAvoidingView style={styles.keyboard} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
            <View accessibilityViewIsModal style={[styles.panel, { backgroundColor: C.background }]}>
              <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
                ...
                <Pressable accessibilityRole="link" onPress={() => { setOpen(false); router.push('/terms'); }} disabled={busy}>
                  <Text style={[styles.paragraph, { color: C.text, textDecorationLine: 'underline' }]}>Үйлчилгээний нөхцөл</Text>
                </Pressable>
                <Text style={[styles.paragraph, { color: C.text }]}>Цуцлах шалтгаан (заавал биш)</Text>
                <TextInput accessibilityLabel="Цуцлах шалтгаан (заавал биш)" value={reason} onChangeText={setReason} editable={!busy}
                  multiline maxLength={1000} style={[styles.input, { color: C.text, borderColor: C.textSecondary }]} />
                <View style={styles.checkbox}>
                  <Switch accessibilityLabel="Буцаалтын нөхцөлтэй танилцлаа." value={accepted} onValueChange={setAccepted} disabled={busy} />
                  <Text style={[styles.paragraph, { flex: 1, color: C.text }]}>Буцаалтын нөхцөлтэй танилцлаа.</Text>
                </View>
                {error ? <Text accessibilityRole="alert" style={styles.dangerText}>{error}</Text> : null}
                <Pressable accessibilityRole="button" disabled={!accepted || busy} accessibilityState={{ disabled: !accepted || busy }}
                  onPress={cancelBooking} style={[styles.confirmButton, (!accepted || busy) && styles.disabled]}>
...
const styles = StyleSheet.create({
  ...
  input: { minHeight: 88, borderWidth: 1, borderRadius: 6, padding: 12, textAlignVertical: 'top', fontSize: 14 },
